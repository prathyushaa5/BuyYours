...
namespace Backend.Controllers{
   ...
   private async Task<Cart> GetOrCreateCartAsync(int userId){
    ...
    var query="select * from Cart where UserID=@userID";
     var cart=await _dbConnection.QueryFirstOrDefaultAsync(query,new {UserID=userId});
     if(cart==null){
        var nextquery="Insert into Cart(UserID,CreatedDate) values(@UserID,@CreatedDate)";
        var  cartId=await _dbConnection.QuerySingleAsync(nextquery,new {UserID=userId,CreatedDate=DateTime.UtcNow});
    ...
    private int GetUserIdFromToken()
        {
            ...
            return 1; // Replace with actual user ID extraction logic
        }
