    ...
    <div className="bg-gray-100 min-h-screen text-gray-800">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6">
        {[image1, image2, image3, image4].map((image, index) => (
          <div key={index} className="bg-white p-6 text-center rounded-lg shadow-md h-60">
            <img src={image} alt={`Category ${index + 1}`} className="w-full h-60 object-cover rounded-lg" />
          </div>
        ))}
      </div>
