...
export default function MainPage() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setDirection(1); // Always slide to the left
  ...
  return (
    <div className="bg-gray-100 min-h-screen text-gray-800">
      {/* Navbar */}
      <nav className="bg-gradient-to-r from-brown-600 to-brown-400 py-4 px-6 flex justify-between items-center shadow-md">
        <h1 className="text-4xl font-extrabold text-white tracking-wide">BuyYours</h1>
      </nav>

      {/* Hero Section */}
      <section className="flex flex-col items-center text-center py-16 bg-brown-500 text-white">
      ...
      <section className="bg-violet-500 text-center py-12 text-white">
        ...
        <div className="flex justify-center">
          ...
          />
          <button className="bg-brown-700 px-8 py-3 text-white font-bold rounded-lg shadow-lg transition-transform transform " >Subscribe</button>
        </div>
      </sectionsss
      <section className="p-8">
      <h3 className="text-4xl font-bold text-center mb-6 text-primary">Discover Our Collection</h3>
